...
export default class JsSignatureProvider implements SignatureProvider {
  ...
  constructor(privateKeys: string[]) {
    for (const k of privateKeys) {
      const pub = convertLegacyPublicKey(
        ecc.PrivateKey.fromString(k)
          .toPublic()
          .toString()
      );
      this.keys.set(pub, k);
  ...
  public async sign({
    ...
    return requiredKeys.map(pub =>
      ecc.Signature.sign(
        signBuf,
        this.keys.get(convertLegacyPublicKey(pub))
      ).toString()
    );
  }
}
